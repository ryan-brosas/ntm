...
#### Flow v2 learning loop — evidence-gated champion/challenger

- Pi and native Codex agents may share one fungible execution pool. Runtime, model, prompt, and tool bundle are experiment attributes, never permanent specialties; every admitted agent still follows the same canonical-ready, one-DRI-through-close contract.
- Keep one immutable **champion** configuration for each declared evaluation window. Admit at most one **challenger** inside the existing WIP-of-three limit, only on a real canonical-ready product Bead. Never create synthetic product work or an idle pane merely to collect a sample.
- Before assignment, record the profile/version, runtime/model, task class, risk, deterministic verifier, and whether the run is champion or challenger. At close, add claim/first-evidence/verify-ready/close timestamps, first-pass verification, rework loops, reassignment or manual intervention, tool degradation, and any later reopen or escaped defect.
- Quality is primary: fresh verification and escaped defects outrank cycle time. Secondary measures are claim-to-close time, verification wait, rework loops, and operator interventions. Commits, lines changed, pane uptime, token volume, and self-reported confidence are not success metrics.
- Learning is batched, not online. Hold the configuration fixed for the window, compare at least three completed runs per arm across at least two representative task classes, count abandoned or reassigned runs as failures, and change only one material variable in a challenger.
- The graph-only planner may compile evidence and propose one bounded experiment or improvement Bead. It may not execute product work, evaluate a Bead it owned, change live routing or prompts mid-window, promote its own proposal, or edit the champion/template. The coordinator names a non-owner evaluator and resolves the recommendation.
- Promote only when the challenger has no ownership, safety, reservation, WIP, or evidence violations; is no worse on first-pass verification and escaped defects; and improves a predeclared flow measure. A tie, missing evidence, or regression retains the champion. Changing live defaults or the reusable template requires an explicit human-approved promotion record linked to Beads, Agent Mail, verification, and commits.
- Dynamically adding a challenger to an existing team, or assigning one after an explicit fresh-team launch, is flow-controlled: do neither while canonical `br ready` is empty, health is not GREEN, implementation WIP is full, or `VERIFY_QUEUED` is full. Tool coverage is team-level; a runtime missing a required tool requests a cited artifact from a capable lane and records the fallback instead of guessing.
